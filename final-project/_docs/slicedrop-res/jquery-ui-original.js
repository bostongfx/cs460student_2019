...
function setupUi() {
    ...
    if (_data.volume.file.length > 0) {
        ...
        jQuery('#threshold-volume').dragslider("option", "values", [volume.min, volume.max]);
        ...
        jQuery('#windowlevel-volume').dragslider("option", "values", [volume.min, volume.max / 2]);
    ...
    if (_data.scalars.file.length > 0) {
        ...
        jQuery("#threshold-scalars").dragslider("option", "min", mesh.scalars.min * 100);
        jQuery("#threshold-scalars").dragslider("option", "max", mesh.scalars.max * 100);
