...
for i,l in enumerate(lines):
    ...
    if l.startswith('element vertex'):                 
        vertexcounter = int(l.split(' ')[-1].strip()) # number of vertex
    elif l.startswith('element face'):
        facecounter = int(l.split(' ')[-1].strip())   #number of indices
    elif l.startswith('end_header'):                  # beginning of vertices
        break

vertex_data = lines[i+1:i+vertexcounter+1]    # last element of vertex
face_data = lines[i+vertexcounter+1:]         # last element of indices

VERTICES = []  
MAX_X = -np.inf
MAX_Y = -np.inf
MAX_Z = -np.inf             # set these to +/- infinity to help find the min and max
MIN_X = np.inf
MIN_Y = np.inf
MIN_Z = np.inf

for v in vertex_data:
    v = [float(v.strip()) for v in v.strip().split(' ')]   # get rid of the extra spaces

    MAX_X = max(MAX_X, v[0])
    MAX_Y = max(MAX_Y, v[1])
    MAX_Z = max(MAX_Z, v[2])
    MIN_X = min(MIN_X, v[0])             # calculate max and min
    MIN_Y = min(MIN_Y, v[1])
    MIN_Z = min(MIN_Z, v[2])    

    VERTICES += v             # add each set of vertices to the array
...
for i in face_data:
    ...
    MAX = max(MAX, i[1])
    MAX = max(MAX, i[2])
    MIN = min(MIN, i[0])                  # again calculate max and mins for faces
    MIN = min(MIN, i[1])
    MIN = min(MIN, i[2])
