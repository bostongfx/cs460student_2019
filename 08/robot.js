...
Robot.prototype.onStep = function() {
  ...
  all_robots.forEach(r => {
    ...
    if (r.root.position.distanceTo(this.root.position) < 10){
...
Robot.prototype.onAnimate = function() {
  ...
  if (this.movement == 'raise left arm') {
    ...
    console.log('a');
    if (typeof this.submovement === 'undefined'){
      console.log('a');
      this.submovement = 'rightstep';
    }
    if (this.submovement == 'rightstep'){
      if (this.right_upperleg.quaternion.w < 0.72) {
  
        // signal that the kick is done and the leg should move back
        this.submovement = 'leftstep';
      }
      else {
        var T = -Math.PI/2;
        this.right_upperleg.quaternion.slerp( new THREE.Quaternion( Math.sin( T / 2 ),   // x
